...
- **Стек:** Python (основной), pandas/Polars для обработки таблиц. Парсинг документов — `unstructured`, `pdfplumber`, Apache Tika. Embeddings — OpenAI `text-embedding-3` или open-source `sentence-transformers`. Векторная БД — Pinecone, Weaviate или Qdrant. Оркестрация — Dagster или Prefect.
- **Скиллы:** ETL, понимание embeddings и chunking, работа с грязными данными, базовый MLOps.
- **Конкуренция:** плотная — Unstructured.io, LlamaIndex, Databricks. Выиграть можно нишей или качеством парсинга.
...
- **Стек:** Python + PyTorch. Видео-диффузия — open-source модели `Stable Video Diffusion`, CogVideoX. GPU — кластеры A100/H100 (аренда через Lambda Labs, RunPod). Для продукта поверх чужих API — Runway API, Pika.
- **Скиллы:** deep learning (диффузия, трансформеры), CUDA, distributed training, видеообработка (ffmpeg, OpenCV).
- **Конкуренция:** тяжёлая — Sora, Runway, Google. Стартапу разумнее строить инструмент редактирования поверх чужих моделей.
...
- **Стек:** Python для ML, time-series модели — `tsfresh`, `sktime`, Prophet, LSTM/Transformer на PyTorch. Мобильное приложение — Swift (iOS) / Kotlin (Android) или React Native. Данные — Apple HealthKit, Google Fit. Бэкенд — FastAPI, БД PostgreSQL/TimescaleDB.
...
- **Стек:** Python для рекомендательных моделей (PyTorch, LightFM). Для цифровых продуктов — React/Next.js с динамическим UI. Для физических — параметрический CAD (Fusion 360 API, Grasshopper для Rhino), 3D-печать.
...
- **Стек:** Python для ML. Generative design — алгоритмы топологической оптимизации. Симуляции прочности (FEA) — софт типа Ansys с API. Computer vision для контроля качества — OpenCV, PyTorch (YOLO для детекции дефектов). CAD-интеграция — STEP-файлы, FreeCAD API.
...
- **Стек:** C# (Unity) или C++ (Unreal Engine). Процедурная генерация — собственные алгоритмы. LLM для логики NPC — локальные модели через `llama.cpp`, Ollama. RL для поведения врагов — Unity ML-Agents (PyTorch под капотом).
...
- **Стек:** Python. Агенты — LangGraph или CrewAI, либо чистые вызовы OpenAI/Anthropic с function calling. RAG — LlamaIndex + векторная БД (Qdrant/Pinecone). Бэкенд — FastAPI. Фронт — Next.js.
